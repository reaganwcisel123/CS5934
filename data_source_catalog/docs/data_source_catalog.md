...
The **County Health Rankings & Roadmaps National County Analytic Data** (`county_health_rankings`) is a public national county and county-equivalent CSV used only by the Rural Care Access Failure Model (`ingestion_wired`). The March 25, 2026 supplemental file contributes Preventable Hospital Stays, Uninsured Adults, three population-to-provider ratios, and Broadband Access. It is optional for normal Virginia dashboard builds; access-failure training requires either the cached official file or a `COUNTY_HEALTH_RANKINGS_PATH` override. Preventable Hospital Stays is an age-adjusted rate per 100,000 Medicare fee-for-service enrollees, so it must not be interpreted as a measure of all residents or as a causal estimate of outpatient care.
